...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 17 19:28:23 2020
...
epi['eta_inj'] = 0.99
epi['alpha_i'] = 1.0
epi['gain'] = lambda n, S: np.max((2140*np.log(n/1.8e18), -5e3))
epi['beta_sp'] = 1e-2
epi['srh_n1'] = epi['ni'] 
epi['srh_p1'] = epi['ni']
epi['srh_tau_n'] = 5e-9
...
# current pulse
J_fun_uni = partial(rect_pulse, t_max=8e-9, width=10e-9, t_rise=1e-9, t_fall=1e-9,
                    amplitude=20/(L*w), constant=0)
z1_fun = lambda t: 1450e-4 - t*8e6
z2_fun = lambda t: 1550e-4 + t*8e6
J_fun = lambda t: J_fun_uni(t) * L/np.min((z2_fun(t)-z1_fun(t), L))
